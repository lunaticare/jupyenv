{
  ...
  inputs.npmlock2nix.url =
    "github:nix-community/npmlock2nix/0ba0746d62974403daf717cded3f24c617622bc7";
  inputs.npmlock2nix.flake = false;
  inputs.opam-nix.url =
    "github:tweag/opam-nix/75199758e1954f78286e7e79c0e3916e28b732b0";
  ...
  outputs = { self, nixpkgs, nixpkgs-stable, flake-compat, flake-utils, ihaskell
    , nix-dart, npmlock2nix, opam-nix, pre-commit-hooks, poetry2nix
    , rust-overlay, }:
    let
      inherit (nixpkgs) lib;
    ...
    in (flake-utils.lib.eachSystem SYSTEMS (system:
      let
        ...
        python = pkgs.python3;
        poetry2nixPkgs =
          import "${poetry2nix}/default.nix" { inherit pkgs poetry; };
        poetry =
          pkgs.callPackage "${poetry2nix}/pkgs/poetry" { inherit python; };

        baseArgs = { inherit self system; };
        ...
        examples =
          kernelLib.mapKernelsFromPath (self + /examples) [ "example" ];
        exampleJupyterlabKernelsNew = (lib.mapAttrs' (name: value:
          lib.nameValuePair ("jupyterlab-kernel-" + name)
          (jupyterLib.mkJupyterlabNew value)) examples);
      ...
      in {
        ...
        apps = {
          update-poetry-lock = flake-utils.lib.mkApp {
